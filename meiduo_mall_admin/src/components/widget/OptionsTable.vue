  ...
  <el-table
      ...
      label="操作">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="fnPopShow(scope.row.id,scope.row.spu_id)">编辑</el-button>
          <el-button
            size="mini"
            type="danger"
    ...
    <el-dialog title="修改规格" :visible.sync="pop_show" append-to-body>
        <el-form :model="optionsForm" status-icon :rules="rulesSpecsForm" ref="optionsForm" label-width="100px">
          <el-form-item label="SPU" prop="spu_id">
            <el-select v-model="optionsForm.spu_id" size="small" @change="optionsForm.specs={}">
              <el-option
  ...
  methods:{
      ...
      this.axios.get(cons.apis + '/specs/options/'+this.edit_id+'/', {
        ...
        .then(dat=>{
            ...
            this.optionsForm.spu_id = dat.data.spu_id;
        }).catch(err=>{
